...
// Permanent only, Band 3+ for support workers, Band 5+ for admin
const CACHE = new Map();
...
function parseNhs(html) {
  ...
  while((m=liRe.exec(html))!==null){
    ...
    const salary=pick(b,'search-result-salary'),
          posted=pick(b,'search-result-publicationDate'),
...
// Fetch from NHS Jobs - contractType=Permanent always, optional salary filter
async function fetchPage(kw, loc, page=1, minSalary=0) {
  const p=new URLSearchParams({keyword:kw, language:'en'});
  if(loc) p.set('location', loc);
  if(page>1) p.set('page', String(page));
  p.set('contractType', 'Permanent'); // Always permanent only
...
// Region location keywords for client-side location filtering
const REGIONS = {
  'London':        ['london'],
  'West Midlands': ['birmingham','coventry','wolverhampton','leicester','derby','stoke','west midlands','sandwell','dudley','walsall'],
  'Wales':         ['cardiff','swansea','newport','wrexham','wales','aberystwyth','bangor'],
  'Manchester':    ['manchester','salford','stockport','oldham','rochdale','bolton','bury','wigan'],
  'West Yorkshire':['leeds','bradford','huddersfield','wakefield','halifax','calderdale','kirklees'],
  'East Yorkshire':['hull','york','east yorkshire','humber','scarborough','beverley'],
};

function inRegion(location, regionName) {
  if(!regionName) return true;
  const l=location.toLowerCase();
  return (REGIONS[regionName]||[]).some(k=>l.includes(k));
}
...
const CATS=[
  // ADMIN - Band 5+ (salary £29,970+)
  {id:'admin-out',label:'Admin Outside London',kw:'administrator',loc:'',exLoc:'london',minBand:4,minSalary:29970,group:'Admin',
  ...
  {id:'admin-lon',label:'Admin in London',kw:'administrator',loc:'London',minBand:4,minSalary:29970,group:'Admin',
    inc:['admin','administrator','administrative','secretary','clerk','receptionist','coordinator','officer','assistant','booking','pathway','clerical','pa to'],exc:CX},

  // SUPPORT WORKERS - Band 3+ (salary £24,071+), permanent, filter by region ourselves
